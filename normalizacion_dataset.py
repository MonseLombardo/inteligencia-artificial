...
Original file is located at
    https://colab.research.google.com/drive/1erkhlKehiW7LfOejpLRSka3y7RxV63uI

Primer cuaderno: Normalizacion de dataset
...
- Normalización de columnas numéricas (Min-Max y Z-Score) aplicada in-place
...
ENCODING = 'latin-1' # Definimos el encoding, probando con 'latin-1' que es común para archivos con caracteres especiales no UTF-8
...
# Conserva la distribución original. Aplicado en su lugar para mantener el número de columnas.
...
print(f'Columnas a normalizar con Min-Max: {cols_normalizar}\n')

scaler_minmax = MinMaxScaler()

for col in cols_normalizar:
    df_clean[col] = scaler_minmax.fit_transform(df_clean[[col]]) # Sobreescribe la columna original
    print(f'   [{col}] → Normalizado Min-Max en su lugar. Rango: [{df_clean[col].min():.4f}, {df_clean[col].max():.4f}]')

print('\n✅ Paso 3 completo | Normalización Min-Max aplicada en su lugar')

# ── PASO 4: Estandarización Z-Score (media=0, std=1) ────────
#
# Ideal para modelos lineales, PCA, SVM.
# Maneja mejor los outliers que Min-Max. Aplicado en su lugar para mantener el número de columnas.

print(f'Columnas a estandarizar con Z-Score: {cols_normalizar}\n')

scaler_std = StandardScaler()

for col in cols_normalizar:
    df_clean[col] = scaler_std.fit_transform(df_clean[[col]]) # Sobreescribe la columna original (y cualquier normalización previa)
    print(f'   [{col}] → Estandarizado Z-Score en su lugar. Media: {df_clean[col].mean():.4f}  Std: {df_clean[col].std():.4f}')

print('\n✅ Paso 4 completo | Estandarización Z-Score aplicada en su lugar')

# ── Resumen final del dataset procesado ─────────────────────

print('── Dataset limpio y normalizado ────────────────────────')
print(f'   Filas:    {df_clean.shape[0]:,}')
print(f'   Columnas: {df_clean.shape[1]}') # Ahora debería ser 25 columnas, las originales
print(f'   Nulos:    {df_clean.isnull().sum().sum()}')

print('\n── Vista previa (columnas original-normalizada) ────────')
# Mostrar algunas columnas numéricas que fueron normalizadas in-place
